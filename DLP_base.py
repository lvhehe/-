...
for i in range(10):
    class SyntheticDataset(DGLDataset):
        ...
        def process(self):
            ...
            self.graphs = []
            self.labels = []
            edge = pd.read_csv('E:\\学习\\科研论文\\DLP\\Dataset\\Data\\brunson_club.csv')
            ...
            for n in index0:
                ...
                if nodes:
                    sub = "根据节点获取局部结构"
                    sub.ndata['x'] = torch.zeros([len(nodes), 4])
    ...
    model = Classifier(4, 25, 1)  # #4是特征的维度，25是隐藏层节点数，2是分类结果
    # print(model)
    ...
    for epoch in range(80):
        ...
        test_rmse /= (iter + 1)
        test_mae /= (iter + 1)
        # print('Epoch {}, loss {:.4f}, rmse {:.4f}, test_rmse {:.4f}, test_mae {:.4f}'.format(epoch, epoch_loss, epoch_rmse,
        #                                                                                  test_rmse, test_mae))
